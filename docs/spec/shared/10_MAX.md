...
## Generated `coll` compatibility

For `.maxpat` artifacts generated by this project's code pipeline and targeting
the certified Max 9.0.0 baseline:

```text
do not manually serialize coll_data
do not depend on coll @embed 1 for generated tables
prefer certified runtime initialization
```

This is a restriction of the certified generation pipeline, not a general Max
limitation. Any future embedded `coll` serialization requires its own
compatibility spike before adoption.
